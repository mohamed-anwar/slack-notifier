...
class Jenkins {
  ...
  getJob(job, cb) {
    const options = Object.assign({
      url: `${this.baseUrl}/job/${job}/api/json`
    }, this.options);
    ...
    request.get(options, (err, res, body) => {
      ...
      if (res.statusCode != 200)
        return cb(new Error(`getJob: got status code: ${res.statusCode}`));

      let jsonBody;
      try {
        jsonBody = JSON.parse(body)
      } catch (e) {
        return cb(new Error(`getJob: failed to parse body: ${e}`))
      }

      cb(null, new JenkinsJob(jsonBody));
    })
  }
  ...
  getUser(user, cb) {
    ...
    request.get(options, (err, res, body) => {
      ...
      try {
        ...
        return cb(`getUser: failed to parse body: ${e}`)
      }
...
class JenkinsBuild {
  ...
  getStartedBy() {
    try {
      return this
        .actions
        .filter(e => e._class == 'hudson.model.CauseAction')[0]
        .causes
        .filter(e => e._class == 'hudson.model.Cause$UserIdCause')[0]
    } catch (e) {
      console.trace(e);
      return null;
    }
